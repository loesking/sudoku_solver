...
from main import validate_value, solve_board
...
class Grid:
    ...
    def __init__(self, rows, cols, width, height):
        ...
        self.width = width
        self.height = height
        self.squares = [[Square(self.test_board[i][j], i, j, width, height) for j in range(cols)] for i in range(rows)]
        self.model = None
        self.selected = None
    ...
    def place(self, val):
        ...
        if self.squares[row][col].value == 0:
            ...
            if validate_value(self.model, val, (row, col)) and solve_board(self.model):
                return True
            else:
